    ...
    <section id="features" className="w-full bg-[#CCFBF1] py-24 md:py-[100px] page-padding">
      ...
      <div ref={cardsRef} className="mx-auto max-w-[1200px] flex flex-col items-center gap-6 md:gap-0">
        {FEATURES.map((feature, i) => (
          <div
            key={i}
            className={`feature-card w-full max-w-[900px] bg-surface border border-silver rounded-3xl p-8 md:p-12 md:-mt-5 first:mt-0 transition-shadow duration-300 hover:shadow-card-hover ${i % 2 === 0 ? '' : 'md:ml-16'
              } ${i === 1 ? 'md:mr-16 md:ml-auto' : ''}`}
          >
            <div
              className={`flex flex-col ${feature.layout === 'right' ? 'md:flex-row-reverse' : 'md:flex-row'
