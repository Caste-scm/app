...
export default function AdminDashboard() {
  ...
  const chartData = [
    { name: 'Lun', visite: stats.visits > 0 ? Math.floor(stats.visits * 0.1) : 10 },
    { name: 'Mar', visite: stats.visits > 0 ? Math.floor(stats.visits * 0.15) : 15 },
    { name: 'Mer', visite: stats.visits > 0 ? Math.floor(stats.visits * 0.2) : 25 },
    { name: 'Gio', visite: stats.visits > 0 ? Math.floor(stats.visits * 0.25) : 35 },
    { name: 'Ven', visite: stats.visits > 0 ? Math.floor(stats.visits * 0.3) : Math.max(10, stats.visits) },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F5] font-sans">
      ...
      <main className="max-w-[1200px] mx-auto p-8">
        <h1 className="text-h3 mb-8">Panoramica</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white p-6 rounded-2xl border border-silver shadow-sm">
            <div className="flex justify-between items-start mb-4">
              <div>
                <p className="text-sm font-semibold text-body uppercase tracking-wider mb-1">Visite Totali</p>
            ...
            <p className="text-xs text-body"><span className="text-green-500 font-medium">{stats.uniqueVisits}</span> visite uniche</p>
          </div>

          <div className="bg-white p-6 rounded-2xl border border-silver shadow-sm">
            <div className="flex justify-between items-start mb-4">
              <div>
                <p className="text-sm font-semibold text-body uppercase tracking-wider mb-1">Ricavo Totale</p>
            ...
            <p className="text-xs text-body">Fondali disponibili su Stripe</p>
          </div>

          <div className="bg-white p-6 rounded-2xl border border-silver shadow-sm">
            <div className="flex justify-between items-start mb-4">
              <div>
                <p className="text-sm font-semibold text-body uppercase tracking-wider mb-1">Ordini Evasibili</p>
            ...
            <p className="text-xs text-body">Da evadere: <span className="font-medium">{stats.orders.filter(o => o.status === 'paid').length}</span></p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-silver shadow-sm">
            <h3 className="font-bold mb-6">Andamento Visite (Ultimi 5 giorni)</h3>
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData}>
                  ...
                  <Line type="monotone" dataKey="visite" stroke="#2dd4bf" strokeWidth={3} dot={{ fill: '#2dd4bf', strokeWidth: 2, r: 4 }} activeDot={{ r: 6 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-silver shadow-sm flex flex-col">
            <h3 className="font-bold mb-4">Ordini Recenti</h3>
                    ...
                    <div key={order.id} className="p-4 border border-silver rounded-xl flex flex-col gap-2">
                      ...
                      {order.status === 'paid' && (
                        <div className="mt-2 pt-2 border-t border-dashed border-silver grid grid-cols-2 gap-4">
                          <div>
                            <p className="text-[10px] font-bold text-charcoal uppercase mb-1">Spedizione:</p>
                          ...
                          <div>
                            <p className="text-[10px] font-bold text-charcoal uppercase mb-1">Fatturazione:</p>
                            ...
                            <div className="mt-2 flex items-center gap-2">
                              <span className="text-[10px] font-bold text-charcoal uppercase">Colore:</span>
